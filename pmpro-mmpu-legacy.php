...
add_action( 'init', 'pmpro_mmpul_init' );
...
function pmpro_mmpu_enqueue_scripts() {
	...
	if ( is_page( $pmpro_pages['levels'] ) || ( !empty( $post->post_content ) && false !== stripos( $post->post_content, '[pmpro_advanced_levels' ) ) ) {
		if ( ! apply_filters( 'pmprommpu_disable_levels_multiselect_page', ! pmpro_mmpul_gateway_supports_multiple_level_checkout() ) ) {
